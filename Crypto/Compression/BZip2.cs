using Yannick.Crypto.Compression.bzip2O;

namespace Yannick.Crypto.Compression
{
    public class BZip2 : CompressionAndDeCompression
    {
        public enum Option
        {
            Fast = 1
        ...
        public BZip2() : base(typeof(Option))
        {
        }
        ...
        public byte[]? Encode(byte[] source) => Encode(source, 1);

        public override byte[]? Encode(byte[] source, int size)
        {
            using (var w = new MemoryStream())
            using (var bzipOutput = new BZip2OutputStream(w, size))
            {
                bzipOutput.Write(source);
                var k = w.GetBuffer();
                return k;
            }
        }
        ...
        public override byte[]? Decode(byte[] source)
        {
            ...
            using (var w = new MemoryStream())
            using (var bzipOutput = new BZip2InputStream(r))
            {
                bzipOutput.CopyTo(w, 4096);
