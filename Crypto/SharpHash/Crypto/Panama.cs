...
namespace Yannick.Crypto.SharpHash.Crypto
{
    ...
    {
        private uint[][] stages;
        private uint[] state, theta, gamma, pi, work_buffer;
        ...
        protected override unsafe void Finish()
        {
            ...
            var padding_size = 32 - (int)(processed_bytes & 31);
            ...
            int a_data_length, int a_index)
        {
            ...
            fixed (uint* thetaPtr = theta, workPtr = work_buffer)
            {
                Converters.le32_copy(a_data, a_index, (IntPtr)workPtr, 0, 32);

                tap16 = (uint)((tap + 16) & 0x1F);

                tap = (tap - 1) & 0x1F;
                tap25 = (uint)((tap + 25) & 0x1F);
