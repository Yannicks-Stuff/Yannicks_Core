...
namespace Yannick.Crypto.SharpHash.Crypto
{
    ...
    {
        private static uint[] table_0 = new uint[256];
        private static uint[] table_1 = new uint[256];
        private static uint[] table_2 = new uint[256];
        private static uint[] table_3 = new uint[256];

        private static readonly uint[] master_table =
        {
        ...
        {
            fixed (uint* t0Ptr = table_0, t1Ptr = table_1, t2Ptr = table_2,
                   t3Ptr = table_3, masterPtr = master_table)
            {
                Utils.Utils.Memmove((IntPtr)t0Ptr, (IntPtr)masterPtr, 256 * sizeof(uint));
        ...
        {
            var HashInstance = new Grindahl256();
        ...
        {
            var padding_size = 12 - (int)(processed_bytes & 3);
            var msg_length = (processed_bytes >> 2) + 1;
