...
namespace Yannick.Crypto.SharpHash.Crypto
{
    internal sealed class RadioGatun64 : BlockHash, ICryptoNotBuiltIn, ITransformBlock
    {
        ...
        {
            var HashInstance = new RadioGatun64();
        ...
        {
            var tempRes = new ulong[4];
            var result = new byte[4 * sizeof(ulong)];

            fixed (ulong* tPtr = tempRes, millPtr = mill)
            {
                fixed (byte* resultPtr = result)
                {
                    ...
                    {
                        RoundFunction();
                        Utils.Utils.Memmove((IntPtr)(tPtr + (i * 2)), (IntPtr)(millPtr + 1), 2 * sizeof(ulong));
                    } // end for
        ...
        {
            var padding_size = 24 - (int)(processed_bytes % 24);
