    ...
    internal abstract class BlockHash : Hash, IBlockHash
    {
        ...
        public override unsafe void TransformBytes(byte[]? a_data, int a_index, int a_length)
        {
            fixed (byte* ptr_a_data = a_data)
            {
                if (!buffer.IsEmpty)
                {
                    if (buffer.Feed((IntPtr)ptr_a_data, a_data.Length, ref a_index, ref a_length, ref processed_bytes))
                        TransformBuffer();
                } // end if

                while (a_length >= buffer.Length)
                {
                    processed_bytes = processed_bytes + (ulong)buffer.Length;
                ...
                if (a_length > 0)
                    buffer.Feed((IntPtr)ptr_a_data, a_data.Length, ref a_index, ref a_length, ref processed_bytes);
            }
