...
namespace Yannick.Crypto.SharpHash.Utils
{
    ...
    {
        public static unsafe void swap_copy_str_to_u32(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            uint* lsrc, ldest, lend;
            byte* lbsrc;
            int lLength;
                  ...
                  dest_index | length) & 3) == 0)
            {
                ...
                lsrc = (uint*)((byte*)(src) + src_index);
                lend = (uint*)(((byte*)(src) + src_index) + length);
                ldest = (uint*)((byte*)(dest) + dest_index);
                while (lsrc < lend)
                {
        ...
        public static unsafe void swap_copy_str_to_u64(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            ulong* lsrc, ldest, lend;
            byte* lbsrc;
            int lLength;
                  ...
                  dest_index | length) & 7) == 0)
            {
                ...
                lsrc = (ulong*)((byte*)src + src_index);
                lend = (ulong*)(((byte*)src + src_index) + length);
                ldest = (ulong*)((byte*)dest + dest_index);
                while (lsrc < lend)
                {
        ...
        public static unsafe void be32_copy(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            if (BitConverter.IsLittleEndian)
            {
        ...
        public static unsafe void be64_copy(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            if (BitConverter.IsLittleEndian)
            {
        ...
        public static uint le2me_32(int x)
        {
            if (!BitConverter.IsLittleEndian)
            {
                return Bits.ReverseBytesUInt32((uint)(x));
            } // end if
        ...
        public static unsafe void le32_copy(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            if (BitConverter.IsLittleEndian)
            {
        ...
        public static unsafe void le64_copy(IntPtr src, int src_index,
            IntPtr dest, int dest_index, int length)
        {
            if (BitConverter.IsLittleEndian)
            {
        ...
        {
            var result = *(uint*)((byte*)a_in + a_index);
            return le2me_32((int)result);
        } // end function ReadBytesAsUInt32LE

        public static unsafe ulong ReadBytesAsUInt64LE(IntPtr a_in, int a_index)
        {
            var result = *(ulong*)((byte*)a_in + a_index);
            return le2me_64(result);
        ...
        public static unsafe string ConvertBytesToHexString(byte[]? a_in, bool a_group, char delimeter = '-')
        {
            ...
            {
                return ConvertBytesToHexString((IntPtr)bPtr, (uint)a_in.Length, a_group, delimeter);
            }
        } // end function ConvertBytesToHexString

        public static string ConvertBytesToHexString(IntPtr a_in, uint size, bool a_group, char delimeter = '-')
        {
            var hex = ExtendedBitConverter.ToString(a_in, 0, (int)size, delimeter);
            hex = hex.ToUpper();
        ...
        public static byte[] ConvertHexStringToBytes(string a_in, char delimeter = '-')
        {
            ...
            var result = new byte[l_in.Length >> 1];

            for (int i = 0, j = 0; i < l_in.Length; i += 2, j += 1)
            {
                var byteStr = l_in.Substring(i, 2);
