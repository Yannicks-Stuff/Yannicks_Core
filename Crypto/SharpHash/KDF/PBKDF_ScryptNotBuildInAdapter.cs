...
namespace Yannick.Crypto.SharpHash.KDF
{
    ...
    internal class PBKDF_ScryptNotBuildInAdapter : KDFNotBuiltIn, IPBKDF_ScryptNotBuiltIn
    {
        ...
        public static readonly string InvalidParallelism =
        ...
        private int Cost, BlockSize, Parallelism;
        ...
        internal PBKDF_ScryptNotBuildInAdapter(byte[] a_PasswordBytes, byte[] a_SaltBytes,
            int a_Cost, int a_BlockSize, int a_Parallelism)
        {
        ...
        public static void ValidatePBKDF_ScryptInputs(int a_Cost, int a_BlockSize,
            int a_Parallelism)
        {
            int LMaxParallel;
            ...
            LMaxParallel = int.MaxValue / (128 * a_BlockSize * 8);
        ...
        private static void ClearArray(ref uint[] a_Input)
        {
            ArrayUtils.ZeroFill(ref a_Input);
        } //
        ...
        private static byte[]? SingleIterationPBKDF2(byte[]? a_PasswordBytes,
            byte[]? a_SaltBytes, int a_OutputLength)
        {
            return (new PBKDF2_HMACNotBuildInAdapter(new SHA2_256(), a_PasswordBytes,
        ...
        /// rotated AValue
        /// </returns>
        private static uint Rotl(uint a_Value, int a_Distance)
        {
        ...
        {
            int LBCount, LIdx, LJdx, LOffset;
            uint LMask;
            uint[] LBlockX1, LBlockX2, LBlockY, LX, LV;

            LBCount = R * 32;
            LBlockX1 = new uint[16];
            LBlockX2 = new uint[16];
            LBlockY = new uint[LBCount];

            LX = new uint[LBCount];

            LV = new uint[N * LBCount];
            ...
            {
                uint[][] temp = { LX, LBlockX1, LBlockX2, LBlockY };
                ClearArray(ref LV);
        ...
        private static void DoSMix(ref uint[] b, int a_Parallelism, int a_Cost,
            int a_BlockSize)
        {
            for (var LIdx = 0; LIdx < a_Parallelism; LIdx++)
                SMix(ref b, LIdx * 32 * a_BlockSize, a_Cost, a_BlockSize);
        } //

        private static unsafe byte[]? MFCrypt(byte[]? a_PasswordBytes, byte[]? a_SaltBytes, int a_Cost,
            int a_BlockSize, int a_Parallelism, int a_OutputLength)
        {
            int LMFLenBytes, LBLen;
            byte[]? LBytes;
            byte[]? result;
            var Lb = new uint[0];
