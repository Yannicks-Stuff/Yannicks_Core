...
namespace Yannick.Crypto.SharpHash.KDF
{
    internal class PBKDF2_HMACNotBuildInAdapter : KDFNotBuiltIn, IPBKDF2_HMACNotBuiltIn
    {
        public static readonly string
            InvalidArgument = "\"bc (ByteCount)\" Argument must be a value greater than zero.";
        ...
        private int BlockSize, startIndex, endIndex;
        private byte[]? buffer;
        private IHMACNotBuiltIn hmacNotBuiltIn;
        private uint IterationCount, Block;
        ...
        internal PBKDF2_HMACNotBuildInAdapter(Interfaces.IHash a_underlyingHash, byte[]? a_password,
        ...
        {
            int LOffset, LSize, LRemainder, LRemCount;
            byte[]? LKey;
            byte[]? LT_Block = null;
        ...
        private byte[]? Func()
        {
            var INT_block = GetBigEndianBytes(Block);
            ...
            var temp = hmacNotBuiltIn.TransformFinal().GetBytes();
            var result = temp.DeepCopy();
